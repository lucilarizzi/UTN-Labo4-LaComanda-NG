...
export class ListadoEncuestasComponent implements OnInit {
  ...
  chartOptions = {
    ...
    title: {
      text: 'Promedio de Satisfaccion'
  },
  xAxis: {
      categories: ['Mozo', 'Cocinero','Restaurant', 'Bebidas']
  },
  ...
  eliminarAnonimos() {
    this.peliculas.forEach((e: Usuario) => {
      ...
      console.log("elim inarnaonimo", e);
  ...
  graficar() {
    ...
    this.peliculas.forEach((e: EncuestaCliente) => {
      console.log("encuesta cliente", this.chartOptions.series);
      contador++;
