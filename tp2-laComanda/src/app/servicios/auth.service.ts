...
export class AuthService {
  ...
  async logInNoGooogle(algo, clave) {
   ...
   val.forEach(async element => {
             ...
             if (element2.email== algo && element2.from == "google") {
               ...
               console.log("this.clavePassSignin" + element2.email, element2.pass);
               await this.clavePassSignin(element2.email, element2.pass).then(() => this.router.navigateByUrl("/perfil"));
             }
            }
            );
  ...
  /*   await credential.user.getIdToken(false).then(e => data.credencial = e);
    console.log(credential.additionalUserInfo.isNewUser);
    console.log(credential.user.getIdToken(false).then(e => console.log(e)));// token    
 */
  ...
  async githubSignin() {
    ...
    const credential = await this.afAuth.auth.signInWithPopup(provider);
    console.log("githus credential", credential);
